...
router.get('/files/download', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: 'Download only supported for local' });
  } catch (err: any) {
...
router.post('/files/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    if (isLocal(id, conn)) {
      ...
      await service.writeFile(remotePath, await fs.promises.readFile(req.file.path, 'utf8'));
      await fs.promises.unlink(req.file.path).catch(() => {});
...
router.post('/files/read', async (req, res) => {
  ...
  try {
    if (isLocal(id, conn)) {
      ...
      const service = conn.service as FtpService;
      const content = await service.readFile(path.posix.join(dirPath || '/', name));
      return res.json({ success: true, content });
    } else if (conn.type === 'sftp') {
      const service = conn.service as SftpService;
      const content = await service.readFile(path.posix.join(dirPath || '/', name));
